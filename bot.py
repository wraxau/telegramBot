...
async def handle_photo(message: types.Message):
    ...
    async with async_session() as db_session:
        ...
        try:
            ...
            # Проверка и обновление лимита после успешного анализа
            result = await db_session.execute(select(Subscription).where(Subscription.user_id == user_id))
            subscription = result.scalars().first()
            if subscription and not subscription.subscription_active:
    ...
    try:
        ...
        async with async_session() as session:
            async with session.begin():
                # Удаляем старую запись для user_id, если она существует
                await session.execute(
                    delete(PendingPayment).where(PendingPayment.user_id == user_id)
                )
        ...
        logger.error(traceback.format_exc())
        await callback_query.message.answer("⚠️ Ошибка при создании платежа. Попробуйте позже.", reply_markup=get_main_keyboard())
        await state.clear()
    await callback_query.answer()
